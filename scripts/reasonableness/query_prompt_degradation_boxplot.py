...
def main():
    ...
    fig_sub, ax_sub = plt.subplots(
        ...
        figsize=(
            (ncols * 3, nrows * 2.5)
            if nrows * ncols > 1
            else (ncols * 4.5, nrows * 4)
        ),
    )
    fig_all, ax_all = plt.subplots()

    if nrows == 1 or ncols == 1:
        ax_sub = np.array([ax_sub])
    ...
    for i, (gt, random) in enumerate(zip(args.gt, args.random)):
        ...
        fig_single.savefig(
            os.path.join(
                args.output_folder,
                f"{args.experiment_names[i]}-{args.metric}.png",
            )
        )
        fig_single.savefig(
            os.path.join(
                args.output_folder,
                f"{args.experiment_names[i]}-{args.metric}.pdf",
            ),
            bbox_inches="tight",
        )
    ...
    fig_all.savefig(
        os.path.join(args.output_folder, f"{args.metric}-degradation.png")
    )
    fig_all.savefig(
        os.path.join(args.output_folder, f"{args.metric}-degradation.pdf"),
        bbox_inches="tight",
    )
    ...
    fig_sub.savefig(os.path.join(args.output_folder, f"{args.metric}-box.png"))
    fig_sub.savefig(
        os.path.join(args.output_folder, f"{args.metric}-box.pdf"),
        bbox_inches="tight",
    )
